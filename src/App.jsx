...
function useData(){
  ...
  useEffect(() => {
    ...
    localStorage.setItem('budgetflow', JSON.stringify({ ...data, version: 52 }));
  }, [data]);
  return [data, setData];
}
...
const makeBackup=()=>{
  ...
  const backupObj = {
    app: 'BudgetFlow',
    version: data?.version ?? 52,
    schemaVersion: data?.schemaVersion ?? 52,
    appVersion: data?.appVersion ?? '52',
...
function SwipeTx({t,c,selected,setSelected,onEdit,onDelete,onDup}){
  ...
  return <div className="swipeShell txSwipeShell">
    ...
    <div className="txCard nativeCard txCardV51 txCardV52" style={{transform:`translateX(${x}px)`}} onTouchStart={start} onTouchMove={move} onTouchEnd={end}>
