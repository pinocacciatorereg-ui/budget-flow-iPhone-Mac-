...
import { Plus, Wallet, Target, ArrowUpCircle, PiggyBank, Trash2, Pencil, Download, Upload, Search, LayoutDashboard, ListChecks, Tags, X, Save } from 'lucide-react';
import './styles.css';
...
function App(){ const [data,setData]=useStore(); const [tab,setTab]=useState('dashboard'); const [month,setMonth]=useState(monthKey()); const [modal,setModal]=useState(null); const cats=data.categories; const tx=data.transactions.filter(t=>t.date?.startsWith(month));
 ...
 return <main><Header tab={tab} setTab={setTab} month={month} setMonth={setMonth} onAdd={()=>setModal({type:'tx'})}/>{tab==='dashboard'&&<Dashboard stats={stats}/>} {tab==='tx'&&<Transactions tx={tx} cats={cats} onEdit={t=>setModal({type:'tx',item:t})} onDelete={delTx} onDeleteMany={delManyTx} onAdd={()=>setModal({type:'tx'})} exportCsv={exportCsv} importCsv={importCsv}/>} {tab==='cats'&&<Categories cats={cats} onEdit={c=>setModal({type:'cat',item:c})} onDelete={delCat} onAdd={()=>setModal({type:'cat'})}/>} {modal?.type==='tx'&&<TxModal item={modal.item} cats={cats} onClose={()=>setModal(null)} onSave={v=>{saveTx(v);setModal(null)}}/>} {modal?.type==='cat'&&<CatModal item={modal.item} onClose={()=>setModal(null)} onSave={v=>{saveCat(v);setModal(null)}}/>}</main> }
function Header(p){return <div className="top"><div className="brand"><div className="logo">▮▮▮</div><h1>BudgetFlow</h1></div><nav>{[['dashboard','Dashboard',LayoutDashboard],['tx','Transazioni',ListChecks],['cats','Categorie',Tags]].map(([id,l,I])=><button className={p.tab===id?'active':''} onClick={()=>p.setTab(id)}><I size={16}/>{l}</button>)}</nav><input type="month" value={p.month} onChange={e=>p.setMonth(e.target.value)}/><button className="add" onClick={p.onAdd}><Plus size={18}/> Aggiungi</button></div>}
function Dashboard({stats}){ const cards=[['Uscite totali',stats.spent,`${stats.budget?Math.round(stats.spent/stats.budget*100):0}% del budget`,Wallet,'blue'],['Budget mensile',stats.budget,'Somma budget categorie',Target,'green'],['Rimanente',stats.remaining,`${stats.budget?Math.round(stats.remaining/stats.budget*100):0}% del budget`,PiggyBank,'yellow'],['Accrediti',stats.income,'Entrate del mese',ArrowUpCircle,'purple']]; return <><section className="cards">{cards.map(([a,b,c,I,tone])=><div className={`card ${tone}`}><I/><span>{a}</span><strong>{eur(b)}</strong><small>{c}</small></div>)}</section><section className="grid"><div className="panel"><h2>Distribuzione spese</h2><div className="chartrow"><ResponsiveContainer width="50%" height={280}><PieChart><Pie data={stats.byCat.filter(c=>c.spent>0)} dataKey="spent" innerRadius={70} outerRadius={115} paddingAngle={2}>{stats.byCat.map(c=><Cell fill={c.color}/>)}</Pie><Tooltip formatter={eur}/></PieChart></ResponsiveContainer><div className="legend">{stats.byCat.map(c=><div><i style={{background:c.color}}></i><span>{c.name}</span><b>{eur(c.spent)}</b><em>{c.share.toFixed(1)}%</em></div>)}</div></div></div><div className="panel"><h2>Spese per categoria</h2><ResponsiveContainer width="100%" height={330}><BarChart layout="vertical" data={stats.byCat} margin={{left:70,right:30}}><CartesianGrid strokeDasharray="3 3"/><XAxis type="number" tickFormatter={v=>v+' €'}/><YAxis dataKey="name" type="category" width={130}/><Tooltip formatter={eur}/><Bar dataKey="spent">{stats.byCat.map(c=><Cell fill={c.color}/>)}</Bar></BarChart></ResponsiveContainer></div></section><section className="bottom"><div className="panel wide"><h2>Riepilogo per categoria</h2><Table rows={stats.byCat}/></div><div className="panel saldo"><h2>Saldo mese</h2><p><span>Accrediti</span><b>{eur(stats.income)}</b></p><p><span>Uscite</span><b>{eur(stats.spent)}</b></p><hr/><p><span>Saldo</span><strong className={stats.balance>=0?'ok':'bad'}>{eur(stats.balance)}</strong></p></div></section><BudgetVsSpentChart rows={stats.byCat}/></>}
...
function Transactions({tx,cats,onEdit,onDelete,onDeleteMany,onAdd,exportCsv,importCsv}){ const [q,setQ]=useState(''); const [selected,setSelected]=useState([]); const list=tx.filter(t=>(t.description||'').toLowerCase().includes(q.toLowerCase())); const shownIds=list.map(t=>t.id); const allShown=shownIds.length>0&&shownIds.every(id=>selected.includes(id)); const toggle=id=>setSelected(s=>s.includes(id)?s.filter(x=>x!==id):[...s,id]); const toggleAll=()=>setSelected(s=>allShown?s.filter(id=>!shownIds.includes(id)):[...new Set([...s,...shownIds])]); const bulkDelete=()=>{onDeleteMany(selected);setSelected([])}; return <div className="panel"><div className="bar"><h2>Transazioni</h2><div><button onClick={exportCsv}><Download size={16}/>Esporta CSV</button><label className="importBtn"><Upload size={16}/>Importa CSV<input type="file" accept=".csv,text/csv" onChange={e=>{importCsv(e.target.files?.[0]); e.target.value='';}}/></label>{selected.length>0&&<button className="danger" onClick={bulkDelete}><Trash2 size={16}/>Elimina selezionate ({selected.length})</button>}<button onClick={onAdd}><Plus size={16}/>Nuova</button></div></div><label className="search"><Search size={16}/><input placeholder="Cerca descrizione" value={q} onChange={e=>setQ(e.target.value)}/></label><table><thead><tr><th><input type="checkbox" checked={allShown} onChange={toggleAll} aria-label="Seleziona tutte le transazioni visibili"/></th><th>Data</th><th>Descrizione</th><th>Categoria</th><th>Tipo</th><th>Importo</th><th></th></tr></thead><tbody>{list.map(t=><tr className={selected.includes(t.id)?'selected':''}><td><input type="checkbox" checked={selected.includes(t.id)} onChange={()=>toggle(t.id)} aria-label={`Seleziona ${t.description}`}/></td><td>{t.date}</td><td>{t.description}</td><td>{cats.find(c=>c.id===t.categoryId)?.name||'Entrata'}</td><td>{t.type==='income'?'Entrata':'Uscita'}</td><td className={t.type==='income'?'ok':'bad'}>{eur(t.amount)}</td><td><button onClick={()=>onEdit(t)}><Pencil size={15}/></button><button onClick={()=>onDelete(t.id)}><Trash2 size={15}/></button></td></tr>)}</tbody></table></div>}
